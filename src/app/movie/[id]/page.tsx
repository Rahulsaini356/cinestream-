...
export default async function MovieDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const getProviderLink = (name: string) => {
    return providerLinks[name] || `https://www.google.com/search?q=watch+${encodeURIComponent(movie.title)}+online`;
  };
            ...
            <div className="flex-1 mt-8">
              ...
              <div className="flex flex-wrap gap-2 mb-8">
                {movie.genres?.map((g: any) => (
                  <Link key={g.id} href={`/movies?genre=${g.id}`} className="px-3 py-1 bg-white/10 hover:bg-white/20 text-white rounded-full text-sm transition-colors backdrop-blur-md">
                    {g.name}
                  </Link>
                ))}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-4">
                ...
                />
                {movie.imdb_id && (
                  <a href={`https://www.imdb.com/title/${movie.imdb_id}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-6 py-3 font-semibold rounded-xl bg-yellow-500/20 text-yellow-500 hover:bg-yellow-500/30 transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-zinc-900/50 backdrop-blur border border-white/5 p-6 rounded-2xl space-y-6 min-h-[250px]">
              ...
              {providers?.rent && (
                <div>
                  <h3 className="text-sm font-medium text-zinc-400 mb-3 uppercase tracking-wider">Rent / Buy</h3>
                  <div className="flex flex-wrap gap-3">
                    {providers.rent.map((p: any) => (
                      <a href={getProviderLink(p.provider_name)} target="_blank" rel="noreferrer" key={p.provider_id} className="w-10 h-10 rounded-lg overflow-hidden hover:scale-110 active:scale-95 transition-all cursor-pointer ring-1 ring-white/10 hover:ring-white/30" title={p.provider_name}>
                        <img src={getImageUrl(p.logo_path, "w500")} alt={p.provider_name} />
                      </a>
                    ))}
                  </div>
            ...
            <div className="flex gap-4 overflow-x-auto pb-6 scrollbar-hide snap-x snap-mandatory">
              {movie.similar.results.map((item: any) => (
                ...
                >
                  ...
                  <h3 className="font-medium text-sm text-zinc-100 truncate mt-1 group-hover:text-white transition-colors">{item.title}</h3>
                </Link>
              ))}
            </div>
          </div>
